...
## 2026-06-16（v3.10.10 补充修复）

### 已完成 ✅
- [x] **修复 build.js 版本号正则缺陷**：原正则只匹配 `vX.Y` 两段，导致 title 为 `v3.10.10` 时构建后变成 `v3.10.10.10`；已改为支持任意段数版本号
- [x] **重新部署 v3.10.10**：构建后验证服务器 title 正确显示为 `BTI 反差人格测试 v3.10.10`
- [x] **Git commit + push**：提交并推送到 GitHub
