...
## 2026-06-16（v3.10.10 修复）

### 已完成 ✅
- [x] **修正首页版本号**：更新 `index.html` title 为 v3.10.10，避免 build.js 正则匹配后变成 v3.10.9.3
- [x] **加深被邀请欢迎页文字颜色**：`app.js` 中 inviteTip 的 slogan 和提示文字颜色从 rgba(255,255,255,.4~.6) 改为更醒目的 #e2e8f0/#cbd5e1
- [x] **加深默契度对比页文字颜色**：`app.js` 中 comparePage 的标题、标签、描述等文字颜色从 #888/#aaa/#ccc 改为 #d1d5db/#e5e7eb/#f3f4f6
- [x] **更新版本号与备份**：index.html 版本注释更新为 v3.10.10，备份 `versions/index_v3.10.10.html`
- [x] **构建打包部署 v3.10.10**：node build.js v3.10.10 + pack.py + deploy.py，35/35 文件上传成功
- [x] **Git commit + push**：提交并推送到 GitHub
...
## 2026-06-16（v3.10.11 修复）

### 已完成 ✅
- [x] **进一步加深邀请页和默契页文字对比度**：`app.js` 中把相关文字改为白色/金色并加粗，提升对比度
- [x] **同步根目录跳转页版本号**：用 paramiko 更新服务器 `/var/www/html/index.html`，使 `wangwangtt.top` 也显示 v3.10.11
- [x] **更新版本号与备份**：index.html 版本注释更新为 v3.10.11，title 更新为 v3.10.11，备份 `versions/index_v3.10.11.html`
- [x] **构建打包部署 v3.10.11**：node build.js v3.10.11 + pack.py + deploy.py，35/35 文件上传成功
- [x] **Git commit + push**：提交并推送到 GitHub
...
### 版本
- 当前：v3.10.11（已部署）
- 访问地址：https://wangwangtt.top/ 或 http://8.148.82.169/bti/
- 访问地址：http://8.148.82.169/bti/
- 备注：如仍看到 V3.6，请清除微信/浏览器缓存或强制刷新
