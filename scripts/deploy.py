...
使用方式：
  ...
  2. 运行：python deploy.py <zip文件路径> <子目录名>
"""
...
REMOTE_ROOT = os.environ.get("DEPLOY_REMOTE_ROOT", "/var/www/html")
...
def validate_config():
    ...
    if not USER:
        missing.append("DEPLOY_USER")
    if not PASS:
        missing.append("DEPLOY_PASS")
    if missing:
        print(f"[x] 缺少必填配置项: {', '.join(missing)}", file=sys.stderr)
        print(f"[i] 请在 .env 文件中设置这些变量，或设置对应的环境变量", file=sys.stderr)
        print(f"[i] 参考: {Path(__file__).parent / '.env.example'}", file=sys.stderr)
        sys.exit(1)
